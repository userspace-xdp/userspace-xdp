...
const struct ip_mac_pair targets_map[MAX_TARGET_COUNT] = {
	{
		.mac = {{ 0xb8, 0x3f, 0xd2, 0x2a, 0xe5, 0x11 }},
		.ip = 0xc0a80102,
	},
	{
		.mac = {{ 0xb8, 0x3f, 0xd2, 0x2a, 0xe5, 0x11 }},
...
static void *bpf_map_lookup_targets_map(const void *key) {
	return (void *)&targets_map[*(uint32_t *)key];
}
...
const struct ip_mac_pair config_map[MAX_TARGET_COUNT] = {
	{
		.mac = {{ 0xb8, 0x3f, 0xd2, 0x2a, 0xe5, 0x11 }},
		.ip = 0xc0a8010b,
	},
...
static void *bpf_map_lookup_config_map(const void *key) {
	return (void *)&config_map[*(uint32_t *)key];
}
...
int bpf_main(void *ctx_base)
{
	...
	if (ip->protocol == IPPROTO_TCP)
	{
		...
		uint32_t key = 1;
		struct ip_mac_pair *client_cfg = bpf_map_lookup_config_map(&key);
		if (!client_cfg)
		{
		...
		if (hash_and_sum_res.xxhash64_res / 2)
		{
			...
			dst = bpf_map_lookup_targets_map(&key);
			if (!dst)
				return XDP_ABORTED;

			key = 0;
			// src = bpf_map_lookup_elem(&config_map, &key);
			src = bpf_map_lookup_config_map(&key);
			if (!src)
				return XDP_ABORTED;
		}
		else
