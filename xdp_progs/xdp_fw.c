...
int main(int argc, char **argv)
{
	...
	flow_map_fd = bpf_map__fd(skel->maps.flow_ctx_table);
	if (flow_map_fd < 0)
	{
		printf("bpf_object__find_map_fd_by_name failed\n");
		return 1;
	...
	// attach
	res = bpf_xdp_attach(ifindex_in, bpf_program__fd(skel->progs.xdp_pass), xdp_flags, NULL);
	if (res)
	{
		fprintf(stderr, "Failed to attach BPF skeleton to %d\n", ifindex_in);
		return 1;
	}
	res = bpf_xdp_attach(ifindex_out, bpf_program__fd(skel->progs.xdp_pass), xdp_flags, NULL);
	if (res)
	{
		fprintf(stderr, "Failed to attach BPF skeleton to %d\n", ifindex_in);
		return 1;
